...
class ScanResultTile extends StatelessWidget {
  ...
  String getNiceServiceData(Map<Guid, List<int>> data) {
    ...
    List<String> res = [];
    data.forEach((id, bytes) {
      res.add('${id.str}: ${getNiceHexArray(bytes)}');
    });
    return res.join(', ');
  }
  ...
  Widget build(BuildContext context) {
    return ExpansionTile(
      ...
      trailing: !result.device.isConnected
          ? Text(
                  BlueToothConnect.instance.isConnectDevice(result)
                      ? '已断开'
                      : "未连接",
                  style: TextStyle(color: AppColors.title))
              .padding(horizontal: 40.h, vertical: 15.w)
              .roundedBorder(radius: 40.r)
              .onTap(onTap)
          : Text(
              '已连接',
              ...
              .padding(horizontal: 40.h, vertical: 15.w)
              .backgroundBorderRadius(color: AppColors.commonPrimary, all: 40.r)
              .onTap(onTap),
      children: <Widget>[
        _buildAdvRow(
